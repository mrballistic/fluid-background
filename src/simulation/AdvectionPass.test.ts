/**
 * Unit tests for AdvectionPass class
 */
...
describe('AdvectionPass', () => {
  let gl: WebGL2RenderingContext;
  let shaderManager: ReturnType<typeof createMockShaderManager>;
  let advectionPass: AdvectionPass;

  beforeEach(() => {
    gl = createMockGL();
    shaderManager = createMockShaderManager();
    advectionPass = new AdvectionPass(gl, shaderManager);
  });
  ...
  describe('execute', () => {
    it('should execute advection pass with proper uniforms', () => {
    ...
    it('should bind velocity and density textures', () => {
      ...
      expect(gl.bindTexture).toHaveBeenCalledWith(gl.TEXTURE_2D, mockInputs.density);
    });
    ...
    it('should throw error if not properly initialized', () => {
      const uninitializedPass = new AdvectionPass(gl, shaderManager);
      ...
      expect(() => uninitializedPass.execute(gl, mockInputs)).toThrow(
        'AdvectionPass not properly initialized'
      );
    });
  });
  ...
  describe('cleanup', () => {
    ...
    it('should handle cleanup when resources are null', () => {
      ...
      expect(() => advectionPass.cleanup()).not.toThrow();
    });
  });
});